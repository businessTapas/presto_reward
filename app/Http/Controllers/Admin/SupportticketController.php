...
class SupportticketController extends Controller
{
   public $page = 'ticket';
    ...
    public function ticket_view($id)
    {
        ...
        $view_ticet = CustomerEnquire::with('customer')->where('id', $id)->first();
    ...
    public function comments(Request $request)
    {
        ...
        if($request -> ajax())
        {
            ...
            ->addColumn('status', function ($row) {
                ...
                return '<div class="form-check form-switch form-switch-md mb-2">
                <input class="form-check-input" type="checkbox" id="toggleSwitch_' . $row->id . '" ' . $checked . ' onclick="changeStatus(\'' . route('company.status', ['id' => $row->id]) . '\', \'status' . $row->id . '\')">
                            <label class="form-check-label" for="toggleSwitch_' . $row->id . '"></label>
                        </div>';
            })
            ->addColumn('action', function ($row) {
